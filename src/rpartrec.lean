...
| (e+5) :=
  ...
  have IH1 : _ := encode_of_nat_code e.div2.div2.div2.unpair.1,
  have IH2 : _ := encode_of_nat_code e.div2.div2.div2.unpair.2,
  have IH3 : _ := encode_of_nat_code e.div2,
  begin
    suffices :
    ...
    cases e.bodd; simp [encode_code, of_nat_code, nat.bit, IH, IH1, IH2],
    ...
    { have h : ∀ m : ℕ, cond m.bodd bit1 bit0 m.div2 = m,
      { intros m, exact nat.bit_decomp m }, simp[h, IH3] }
  end
...
| (k+1) f zero         := λ n, guard (n ≤ k) >> pure 0
| (k+1) f succ         := λ n, guard (n ≤ k) >> pure (nat.succ n)
| (k+1) f left         := λ n, guard (n ≤ k) >> pure n.unpair.1
| (k+1) f right        := λ n, guard (n ≤ k) >> pure n.unpair.2
...
| (s+1) (pair cf cg) f g := λ h n y,
    have IH0 : _ := @evaln_inclusion (s+1) cf _ _ h,
    have IH1 : _ := @evaln_inclusion (s+1) cg _ _ h,
    by { simp[evaln, (>>)], assume e hf,
         have : ∃ y, evaln (s + 1) f cf n = some y, 
         { cases evaln (s + 1) f cf n; simp, simp[(<*>), hf] at hf ⊢, exact hf },
         rcases this with ⟨y0, hy0⟩, have IH0' := IH0 _ _ hy0,
          have : ∃ y, evaln (s + 1) f cg n = some y, 
         { cases evaln (s + 1) f cg n; simp, simp[(<*>), hf] at hf ⊢, exact hf },
         rcases this with ⟨y1, hy1⟩, have IH1' := IH1 _ _ hy1,
         simp[hy0, hy1, IH0', IH1'] at hf ⊢, exact ⟨e, hf⟩ }
| (s+1) (comp cf cg) f g := λ h n y,
    have IH0 : _ := @evaln_inclusion (s+1) cf _ _ h,
    have IH1 : _ := @evaln_inclusion (s+1) cg _ _ h,
    by { simp[evaln, (>>)], assume e z hz hy,
         refine ⟨e, z, IH1 _ _ hz, IH0 _ _ hy⟩ }
| (s+1) (prec cf cg) f g := λ h n y,
    have l0 : ∀ x y, x < s → f x = some y → g x = some y :=
      λ x y e, h x y (nat.lt.step e),
    have IH0 : _ := @evaln_inclusion s (cf.prec cg) _ _ l0,
    have IH1 : _ := @evaln_inclusion (s+1) cf _ _ h,
    have IH2 : _ := @evaln_inclusion (s+1) cg _ _ h,
    by { simp[evaln, (>>)], assume e hf,
         cases n.unpair.snd with n0; simp at hf ⊢, exact ⟨e, IH1 _ _ hf⟩,
         rcases hf with ⟨z, hz0, hz1⟩,
         refine ⟨e, z, IH0 _ _ hz0, IH2 _ _ hz1⟩ }
| (s+1) (rfind' cf)  f g := λ h n y, 
    have l0 : ∀ x y, x < s → f x = some y → g x = some y :=
      λ x y e, h x y (nat.lt.step e),
    have IH0 : _ := @evaln_inclusion (s+1) cf _ _ h, 
    have IH1 : _ := @evaln_inclusion s cf.rfind' _ _ l0,
    by { simp[evaln, (>>)], assume e z ez ey,
         refine ⟨e, z, IH0 _ _ ez, _⟩,
         cases z with z0; simp at ey ⊢, exact ey, exact IH1 _ _ ey }
| (s+1) (univn cf)  f g := λ h n y, 
    have IH0 : _ := @evaln_inclusion (s+1) cf _ _ h,
    have IH0 : _ := @evaln_inclusion
      (min s n.unpair.fst.unpair.fst) (of_nat code n.unpair.fst.unpair.snd)
      (evaln (s + 1) f cf) (evaln (s + 1) g cf) (λ x y _, IH0 x y),
    by { simp[evaln, (>>)], assume e ey, refine ⟨e, IH0 _ _ ey⟩ }
...
| (s+1) (pair cf cg) f g := λ h,
    have IH0 : _ := @evaln_use (s+1) cf _ _ h,
    have IH1 : _ := @evaln_use (s+1) cg _ _ h,
    funext $ λ n, 
...
| (s+1) (prec cf cg) f g := λ h,
    ...
    have IH0 : _ := @evaln_use s (cf.prec cg) _ _ l0,
    have IH1 : _ := @evaln_use (s+1) cf _ _ h,
    have IH2 : _ := @evaln_use (s+1) cg _ _ h,
    funext $ λ n, 
    by { simp[evaln], have : n ≤ s ∨ ¬ n ≤ s, omega, cases this;
         simp[(>>), this, IH0, IH1, IH2] }
| (s+1) (rfind' cf)  f g := λ h, 
    have l0 : ∀ (x : ℕ), x < s → f x = g x := λ x e, h x (nat.lt.step e),
    have IH0 : _ := @evaln_use (s+1) cf _ _ h,
    have IH1 : _ := @evaln_use s cf.rfind' _ _ l0,
    funext $ λ n, 
    by { simp[evaln], have : n ≤ s ∨ ¬ n ≤ s, omega, cases this;
         simp[(>>), this, IH0, IH1] }
| (s+1) (univn cf)   f g := λ h, funext $ λ n,
    have IH0 : _ := @evaln_use (s+1) cf _ _ h,
    have IH0 : _ := @evaln_use
      (min s n.unpair.fst.unpair.fst) (of_nat code n.unpair.fst.unpair.snd)
      (evaln (s + 1) f cf) (evaln (s + 1) g cf) (by simp[IH0]),
    by { simp[evaln], have : n ≤ s ∨ ¬ n ≤ s, omega, cases this;
         simp[(>>), this, IH0] }
...
| (s₀+1) (s₁+1) (pair cf cg) f n x := λ hs,
    have IH0 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cf f n y hs,
    have IH1 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cg f n y hs,
    by { simp[evaln, (>>)], assume e0 ex,
         ...
         rcases this with ⟨y₁, hy₁⟩,
         simp[hy₀, hy₁, IH0 _ _ hy₀, IH1 _ _ hy₁] at ex ⊢,
         exact ⟨le_trans e0 (nat.le_of_succ_le_succ hs), ex⟩ }
| (s₀+1) (s₁+1) (comp cf cg) f n x := λ hs,        
    have IH0 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cf f n y hs,
    have IH1 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cg f n y hs,
    by { simp[evaln, (>>)], assume e0 y ey ex,
         refine ⟨le_trans e0 (nat.le_of_succ_le_succ hs), y, IH1 _ _ ey, IH0 _ _ ex⟩ }
| (s₀+1) (s₁+1) (prec cf cg) f n x := λ hs,
    have IH0 : _ := λ n y, @evaln_mono s₀ s₁ (cf.prec cg) f n y (nat.le_of_succ_le_succ hs),
    have IH1 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cf f n y hs,
    have IH2 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cg f n y hs,
    by { simp[evaln, (>>)],
         cases n.unpair.snd with n0; simp,
         assume e0 ex,
         exact ⟨le_trans e0 (nat.le_of_succ_le_succ hs), IH1 _ _ ex⟩,
         assume e0 y ey ex,
         refine ⟨le_trans e0 (nat.le_of_succ_le_succ hs), y, IH0 _ _ ey, IH2 _ _ ex⟩ }
| (s₀+1) (s₁+1) (rfind' cf)  f n x := λ hs,
    have IH0 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cf f n y hs,
    have IH1 : _ := λ n y, @evaln_mono s₀ s₁ cf.rfind' f n y (nat.le_of_succ_le_succ hs),
    by { simp[evaln, (>>), pure],
         assume e0 y ey ex,
         refine ⟨by omega, y, IH0 _ _ ey, _⟩,
         cases y with y0; simp at ex ⊢, exact ex, exact IH1 _ _ ex }
| (s₀+1) (s₁+1) (univn cf)   f n x := λ hs,
    have IH0 : _ := λ n y, @evaln_mono (s₀+1) (s₁+1) cf f n y hs,
    have IH1 : _ := λ n y : ℕ, @evaln_mono
    ...
    by { simp[evaln, (>>)], assume e ex,
         ...
         have := IH1 _ _ this ex, refine ⟨by omega, _⟩,
         exact evaln_inclusion (λ x y _, IH0 x y) _ _ this }
...
theorem evaln_sound {f} : ∀ {k c n x},
  x ∈ evaln k f c n → x ∈ eval f c n
| 0     _ n x h := by simp [evaln] at h; cases h
| (k+1) c n x h := begin
  induction c with cf cg hf hg cf cg hf hg cf cg hf hg cf hf cf hf generalizing x n;
    simp [eval, evaln, (>>), (<*>)] at h ⊢, 
  { exact h.2 },
  { simp [pure, pfun.pure, eq_comm] at h ⊢, exact h.2 },
  { simp [pure, pfun.pure, eq_comm] at h, exact h.2 },
  { simp [pure, pfun.pure, eq_comm] at h, exact h.2 },
  { simp [pure, pfun.pure, eq_comm] at h, exact h.2 },
  { rcases h with ⟨_, y, ef, z, eg, rfl⟩,
    exact ⟨_, hf _ _ ef, _, hg _ _ eg, rfl⟩ },
  { rcases h with ⟨_, y, eg, ef⟩,
  ...
  { apply eval_univn_evaln_iff.mpr, use k, simp[evaln, (>>)], exact h },
end
...
instance {α β} : has_coe (α → option β) (α →. β) := ⟨λ f, (λ x, f x)⟩

inductive rpartrec (o : ℕ → option ℕ) : (ℕ →. ℕ) → Prop
...
| univn {f : ℕ → option ℕ} : rpartrec f → rpartrec (unpaired $ λ a n, 
    nat.rpartrec.code.evaln a.unpair.1 f (of_nat _ a.unpair.2) n)

namespace rpartrec

def reducible (f g) : Prop := rpartrec g f
local infix ` pr_in `:80 := reducible

theorem of_eq {o} {f g : ℕ →. ℕ} (hf : rpartrec o f) (H : ∀ n, f n = g n) : rpartrec o g :=
(funext H : f = g) ▸ hf

theorem of_partrec {f} (g) (hf : nat.partrec f) : f pr_in g :=
begin
  induction hf,
...
theorem of_primrec {f} (g) (hf : primrec f) : f pr_in g := 
of_partrec g (partrec.of_primrec hf)

protected theorem some {f} : roption.some pr_in f := of_primrec f primrec.id

theorem none {f} : (λ n, none) pr_in f := of_partrec f partrec.none

theorem prec' {f g h o} (hf : f pr_in o) (hg : g pr_in o) (hh : h pr_in o) :
  (λ a, (f a).bind (λ n, n.elim (g a) (λ y IH, do i ← IH, h (mkpair a (mkpair y i))))) pr_in o :=
((prec hg hh).comp (pair rpartrec.some hf)).of_eq $ λ a, ext $ λ s, by simp [(<*>)]

theorem refl {f : ℕ → option ℕ} : f pr_in f := oracle

theorem trans {f g h : ℕ → option ℕ} : f pr_in g → g pr_in h → f pr_in h :=
begin
  assume pgf phg,
  induction pgf,
  case oracle { exact phg },
  case univn : f hf pf { exact univn pf },
  case zero { exact zero },
  case succ { exact succ },
  case left { exact left },
  case right { exact right },
  case pair : f g hf hg pf pg { exact pair pf pg },
  case comp : f g hf hg pf pg { exact comp pf pg },
  case prec : f g hf hg pf pg { exact prec pf pg },
  case rfind : f pf hf { exact rfind hf },
end

end rpartrec

end nat
def pfun_opt  (α : Type*) (β : Type*) := α → option β
infixr ` →o `:25 := pfun_opt

def rpartrec {α β σ τ} [primcodable α] [primcodable β] [primcodable σ] [primcodable τ] 
  (f : α →. σ) (g : β → option τ) := nat.rpartrec.reducible
(λ n, roption.bind (decode α n) (λ a, (f a).map encode))
(λ n, option.bind (decode β n) (λ a, (g a).map encode))
infix ` pr_in `:80 := rpartrec

def rcomputable {α β σ τ} [primcodable α] [primcodable β] [primcodable σ] [primcodable τ]
  (f : α → σ) (g : β → option τ) := (f : α →. σ) pr_in g
