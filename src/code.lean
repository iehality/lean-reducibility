...
(code.evaln_ropt s 
  (λ n, roption.of_option ((decode β n).map (λ a, encode (f a))))
...
begin
  ...
  have : (λ (n : ℕ), ↑(decode α n).bind (λ (a : α), roption.map encode (f a))) = 
    
    --rw [this],
  
  split,
  { assume h,have := nat.rpartrec.code.exists_code.mp h,
    
    simp only [nat.rpartrec.code.exists_code], 
